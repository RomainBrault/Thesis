...
counterpart when $r=2Du'$ is small compared to $(N+U)u$. Roughly speaking it is
better to use \cref{alg:close_form} when the number of features, $r$, required
is small compared to the number of training points. Notice that computing the
...
vector-valued function has increased the (expected) complexity from $p$ to
