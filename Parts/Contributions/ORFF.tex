...
$\tildef{1:D}(x)=\tildePhi{\omega}(x)^\adjoint \theta$, where $\theta$ lives in some redescription space\mpar{If $V=1$, $\mathcal{U}=\mathbb{R}$ then $\mathcal{U}'=\mathbb{R}$ thus $\theta\in \mathbb{R}^D$ endowed with the euclidean inner product.} $\tildeH{\omega} = \Vect_{j=1}^D\mathcal{U}'$. We are interested in finding a parameter vector $\theta_{\seq{z}}$ such that
\begin{dmath}
...
\end{dmath*}
Where the two last equalities apply since $\mathcal{H}_0$, $\mathcal{H}_1$, $\mathcal{H}_2$ are a Hilbert spaces, and thus are reflexive. By linearity and applying the chaine rule to \cref{eq:argmin_theta} and since $M_{ik}^\adjoint = M_{ki}$ for all $i$, $k\in\mathbb{N}_{N+U}$, we have
\begin{dgroup*}
\begin{dmath*}
\lderivative{\frac{\lambda_K}{2}\norm{\theta}^2_{\Vect_{j=1}^D\mathcal{U}'}}{\theta}=\lambda_K\theta,
\end{dmath*}
\begin{dmath*}
\lderivative{\frac{\lambda_M}{2}\sum_{i,k=1}^{N+U}\inner{\tildePhi{\omega}(x_i)^\adjoint \theta, M_{ik}\tildePhi{\omega}(x_k)^\adjoint \theta}_{\mathcal{U}}}{\theta}=\lambda_M\left(\sum_{i,k=1}^{N+U}\tildePhi{\omega}(x_i)M_{ik}\tildePhi{\omega}(x_k)^\adjoint\right) \theta,
\end{dmath*}
\begin{dmath*}
\lderivative{\frac{1}{N}\sum_{i=1}^Nc\left(V\tildePhi{\omega}(x_i)^\adjoint \theta, y_i\right)}{\theta}{\theta}= \frac{1}{N}\sum_{i=1}^N \tildePhi{\omega}(x_i)V^\adjoint \lderivativeat{c\left(y, y_i\right)}{y}{V\tildePhi{\omega}(x_i)^\adjoint \theta}.
\end{dmath*}
\end{dgroup*}
Provided that $c(y,y_i)$ is Frechet differentiable \wrt~$y$, for all $y$ and $y_i\in\mathcal{Y}$. Hence
\begin{dmath*}
\lderivative{J_{\lambda_K}(\theta)}{\theta} = \frac{1}{N}\sum_{i=1}^N \tildePhi{\omega}(x_i)V^\adjoint(Dc)\left(V\tildePhi{\omega}(x_i)^\adjoint \theta, y_i\right) + \lambda_K\theta + \lambda_M\sum_{i,k=1}^{N+U}\tildePhi{\omega}(x_i)M_{ik}\tildePhi{\omega}(x_k)^\adjoint \theta
\end{dmath*}
