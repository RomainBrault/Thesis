...
our attention to the use of ORFF in the framework of supervised and semi-supervised learning. 
%supervised learning function with an ORFF model that approximate an OVK
  \subsection{Warm-up: vector-valued regression} Let $\seq{s} =
(x_i,y_i)_{i=1}^N\in\left(\mathcal{X}\times\mathcal{Y}\right)^N$ be a training random sample. Given a local loss function $L:
\mathcal{X}\times\mathcal{F}\times\mathcal{Y}\to \overline{\mathbb{R}}$ such
that $L$ is proper, convex and lower semi-continous in $\mathcal{F}$, we are interested
...
\end{dmath*}
is called the empirical risk of the model $f\in\mathcal{H}_K$ according the local loss $L$. A common choice of data
fitting term for regression is $L:(x, f, y) \mapsto
\norm{f(x)-y}_{\mathcal{Y}}^2$.  We introduce a corollary from Mazur and
...
of~\citet{minh2016unifying}. This framework encompasses Vector-valued Manifold
