...
\end{proof}
The approximate feature map proposed in \cref{pr:ORFF-map} has direct link with the functional feature map defined in \cref{pr:fourier_feature_map} since we have
\begin{dmath}
\tildePhi(x)y = \frac{1}{\sqrt{D}} \Vect_{j=1}^D (\Phi_x y)(\omega_j).
\end{dmath}
Therefore $\tildePhi(x)$ can be seen as an \say{operator-valued vector} corresponding the \say{stacking} of \iid~operator-valued realization of $\Phi_x$, the functional feature map. Therefore, in light of \cref{pr:regularization}, it is possible to define an approximate feature map of an operator-valued kernel.
