...
\end{proof}
We write $\tildePhi(x)^*\tildePhi(x)\approx K(x,z)$ when $\tildePhi(x)^*\tildePhi(x)\converges{\asurely}{} K(x,z)$ in the weak operator topology when $D$ tends to infinity. With mild abuse of notation we say that $\tildePhi(x)\approx \Phi_x$ when for all $y\in\mathcal{Y}$, $\inner{y, K(x,z)y'}=\inner{\Phi_x y, \Phi_z y'}\approx \inner{\tildePhi(x)y, \tildePhi(x)y'}\coloneqq \tilde{K}(x,z)$ where $\Phi_x$ is defined in the sense of \cref{pr:fourier_feature_map}.
