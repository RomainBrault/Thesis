...
If there exist an operator-valued function $B:\dual{\mathcal{X}}\to\mathcal{L}(\mathcal{Y},\mathcal{Y}')$ such that for all $y$, $y'\in\mathcal{Y}$,
\begin{dmath*}
...
\end{equation}
is an approximated feature map\mpar{\Ie~it satisfies $\tildePhi{1:D}(x)^\adjoint \tildePhi{1:D}(z)\converges{\asurely}{D\to\infty}K(x,z)$ where $K$ is a $\mathcal{Y}$-Mercer \acs{OVK}.} of an \acl{OVK}.
\end{proposition}
\begin{proof}
...
where
\begin{dmath*}
\widetilde{\mathcal{H}}^D = \Set{\Vect_{j=1}^Dg(\omega_j)| \forall g\in L^2\left(\dual{\mathcal{X}}, \probability_{\dual{\Haar},\rho}, \mathcal{Y}'\right), \enskip \omega_j\sim \probability_{\dual{\Haar},\rho} \enskip \text{\iid} }
\subset \Vect_{j=1}^D \mathcal{Y}' ,
\end{dmath*}
is a Hilbert space whose inner product is
\begin{dmath*}
\inner{\theta_1, \theta_2}_{\widetilde{\mathcal{H}}^D} = \sum_{j=1}^D\inner{g_1(\omega_j),g_2(\omega_j)}_{\mathcal{Y}'},
\end{dmath*}
for all $\theta_1=\vect_{j=1}^Dg_1(\omega_j)\in\widetilde{\mathcal{H}}^D$ and $\theta_2=\vect_{j=1}^Dg_2(\omega_j)\in\widetilde{\mathcal{H}}^D$. In particular, notice that for any $x\in\mathcal{X}$ and $y\in\mathcal{Y}$, $\tildePhi{1:D}(x)y\in\widetilde{\mathcal{H}}^D$ as we can take $g(\omega_j)=\sqrt{D}^{-1}\pairing{x,\omega_j}B(\omega_j)^\adjoint y$. Thus we have for all $y$, $y'\in\mathcal{Y}$ and all $x$, $z\in\mathcal{X}$
\begin{dmath*}
...
\end{proposition}
\begin{proof} Apply \cref{pr:feature_operator} to $\widetilde{\Phi}$ to show that $\widetilde{K}$ is an \acs{OVK}. \Cref{pr:kernel_signature} shows that $K$ is shift-invariant since $K(x,z)=K_e(x,\groupop z^{-1})$. Note that $\widetilde{K}$ is not necessarily a $\mathcal{Y}$-Mercer kernel although $\lim_{D\to\infty} \widetilde{K}$ is $\mathcal{Y}$-Mercer.
\end{proof}
...
\begin{dmath*}
(\tildeW{D} \theta)(x) \colonequals \tildePhi{1:D}(x)^\adjoint \theta =\frac{1}{D}\sum_{j=1}^D \conj{\pairing{x,\omega_j}}B(\omega_j)\theta_j, \enskip \omega_j \hiderel{\sim} \probability_{\dual{\Haar},\rho} \enskip\text{\iid}.
\end{dmath*}
where $\theta=\Vect_{j=1}^D\theta_j\in\widetilde{\mathcal{H}}^D$.
\end{definition}
...
\begin{dmath*}
\theta \colonequals \Vect_{j=1}^D \underbrace{\frac{g(\omega_j)}{\sqrt{D}}}_{=\theta_j}, \enskip \omega_j \sim \probability_{\dual{\Haar},\rho} \enskip\text{\iid}.
\end{dmath*}
Then
\begin{propenum}
...
\end{propenum}
where $\omega_j\sim \probability_{\dual{\Haar},\rho}$ \iid.
\end{proposition}
...
\begin{dmath*}
\inner{y, K(x,z)y'}_{\mathcal{Y}}=\inner{\Phi_x y, \Phi_z y'}_{\mathcal{Y}}\approx \inner{\tildePhi{1:D}(x)y, \tildePhi{1:D}(x)y'}_{\mathcal{Y}}\colonequals \inner{y, \tilde{K}(x,z)y'}_{\mathcal{Y}}
\end{dmath*}
...
% \end{sideways}
\caption[Relationships between feature-maps.]{Relationships between feature-maps. $\widetilde{\mathcal{H}}^D = \Set{\Vect_{j=1}^Dg(\omega_j)| \forall g\in L^2\left(\dual{\mathcal{X}}, \probability_{\dual{\Haar},\rho}, \mathcal{Y}'\right), \enskip \omega_j\sim \probability_{\dual{\Haar},\rho} \enskip \text{\iid} }
\subset \Vect_{j=1}^D \mathcal{Y}'$.}
...
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{x,\omega_j}}B(\omega_j)^\adjoint  \\ \sin{\inner{x,\omega_j}}B(\omega_j)^\adjoint \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\dual{\Haar},\rho} \enskip\text{\iid}.
\end{dmath*}
\end{example}
\begin{proof}
...
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{x,\omega_j}}B^\adjoint \\ \sin{\inner{x,\omega_j}}B^\adjoint \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\mathcal{N}(0,\sigma^{-2}I_d)} \enskip\text{\iid}
=\tildephi{1:D}(x)\otimes B^\adjoint,
\end{dmath*}
...
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{x,\omega_j}}\omega_j^T \\ \sin{\inner{x,\omega_j}}\omega_j^T \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\mathcal{N}(0,\sigma^{-2}I_d)} \enskip\text{\iid}
\end{dmath*}
and a bounded \acs{ORFF} map is
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{x,\omega_j}}\frac{\omega_j^T}{\norm{\omega_j}} \\ \sin{\inner{x,\omega_j}}\frac{\omega_j^T}{\norm{\omega_j}} \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\rho} \enskip\text{\iid}.
\end{dmath*}
...
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{x,\omega_j}}B(\omega_j)^T \\ \sin{\inner{x,\omega_j}}B(\omega_j)^T \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\rho} \enskip\text{\iid}
\end{dmath*}
...
\begin{equation*}
\begin{aligned}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{\log(x+c),\omega_j}}B(\omega_j)^\adjoint  \\ \sin{\inner{\log(x+c),\omega_j}}B(\omega_j)^\adjoint \end{pmatrix}, \enskip \omega_j \sim \probability_{\dual{\Haar},\rho} \enskip\text{\iid}.
\end{aligned}
\end{equation*}
\end{example}
\begin{proof}
...
\begin{dmath*}
\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\begin{pmatrix}\cos{\inner{\log(x+c),\omega_j}}B^\adjoint \\ \sin{\inner{\log(x+c),\omega_j}}B^\adjoint \end{pmatrix}, \enskip \omega_j \hiderel{\sim} \probability_{\rho} \enskip\text{\iid}
=\tildephi{1:D}(x) \otimes B^\adjoint,
\end{dmath*}
