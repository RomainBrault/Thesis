...
\cref{eq:argmin_RKHS_rand}, then $f_{\seq{z}}=\Phi(\cdot)^\adjoint
\theta_{\seq{z}}$. To the best of our knowledge no such results exist in the
...
\begin{proof}
    ...
    \end{dmath*}
\end{proof} This shows that when $\lambda_K>0$ the solution of
...
transformation. Namely, if
$u_{\seq{z}}$ is the solution of \cref{eq:argmin_u}, $\theta_{\seq{z}}$ is the
solution of \cref{eq:argmin_theta} and $\lambda_K>0$ we have 
\begin{dmath*}
    \theta_{\seq{z}} = \sum_{j=1}^{N+U} \tildePhi{\omega}(x_j) u_{\seq{z}}
    \hiderel{\in}\tildeH{\omega}.
\end{dmath*} 
If $\lambda_K=0$ we can still find a solution $u_{\seq{z}}$ of
\cref{eq:argmin_u}. By construction of the kernel expansion, we have
$u_{\seq{z}}\in(\Ker W)^\bot$. However looking at the proof of
\cref{th:orff_representer} we see that $\theta_{\seq{z}}$ might \emph{not}
...
\begin{center}
    \begin{algorithm2e}[H]
        ...
        \Input{\begin{itemize}
            ...
            operators (see \cref{th:orff_representer}),
...
\cref{alg:ORFF_construction}, developped in \cref{sec:building_ORFF} and
...
$K_0(\delta)=\exp(-\norm{\delta}/\sigma^2)\Gamma$.  To apply
$\cref{alg:close_form}$ after noticing that in the case of the decomposable
kernel with $\lambda_M=0$, it boilds down to a Stein equation
