...
\begin{theorem}[Representer theorem]
    ...
    hyperparameters The solution $f_{\seq{z}}\in\mathcal{H}_K$ of the
...
$\lambda=0$. However $f_{\seq{z}}$ is no longer the solution of
...
\begin{proof}
    ...
    \begin{dmath*}
        \mathcal{H}_{K, \seq{z}}=\Set{\sum_{j=1}^{N}K_{x_j}u_j| \forall
        (u_i)_{i=1}^{N} \in\mathcal{Y}^{N}}.
    \end{dmath*}
...
\begin{theorem}[Feature equivalence]
    ...
    \theta_{\seq{z}}$, where $\theta_{\seq{s}} \in (\Ker
...
\begin{proof}
    ...
    \end{dmath*}
\end{proof} 
This shows that when $\lambda>0$ the solution of \cref{eq:argmin_u} with the
...
solution of \cref{eq:argmin_theta} and $\lambda>0$ we have 
\begin{dmath*}
    \theta_{\seq{z}} = \sum_{j=1}^{N} \tildePhi{\omega}(x_j) u_{\seq{s}}
...
\begin{dmath*}
    r_{\seq{s}} = \theta_{\seq{s}} - \sum_{j=1}^{N} \tildePhi{\omega}(x_j)
    u_{\seq{s}} \hiderel{\in}\tildeH{\omega}.
\end{dmath*}
Then if $r_{\seq{s}}=0$, it means that $\lambda_K$ is large enough for both
reprensenter theorem and \acs{ORFF} representer theorem to apply. If
$r_{\seq{s}}\neq 0$ but $\tildePhi{\omega}(\cdot)^\adjoint r_{\seq{s}} = 0$ it
means that both $\theta_{\seq{s}}$ and $\sum_{j=1}^{N} \tildePhi{\omega}(x_j)
u_{\seq{s}}$ are in $(\Ker W)^\bot$, thus the representer theorem fails to find
...
kernelized solution $u_{\seq{z}}$ of \cref{th:representer} is
\begin{dmath*}
    \left(\frac{1}{N} \mathbf{K}  + \lambda
    I_{\Vect_{i=1}^{N}\mathcal{Y}}\right)u_{\seq{z}}=\Vect_{i=1}^N y_i.
\end{dmath*}
...
of the similarities between the tasks.
\begin{equation}
