...
$\mathcal{F}$ and called the \emph{hypotheses class} such that the
$f\in\mathcal{F}$ make good predictions for the pair $(X,Y)$ distributed
...
\mathcal{F}$ that minimizes the following criteria, called the true risk
associated to $L$:
...
to all the possible experiments (even the one that are not present in the
training set). Although the convergence of the empirical risk to the true risk
is guaranteed by the strong law of large number, for a given value of $N$, the
...
there exists an optimal solution that minimizes the true risk.
\begin{dmath*}
...
\begin{definition}[Symmetric Positive Definite kernels]
    ...
    is \acf{SPSD}.
\end{definition}
The following proposition gives sufficient conditions to obtain a SPSD matrix:
...
\acs{PSD} kernel defines a unique \acs{RKHS}. Note that the converse is also
true.
...
If we suppose that $L$ is convex in $f(x)$, then it is possible to derive a
