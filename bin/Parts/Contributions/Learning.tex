...
of supervised learning in \acs{vv-RKHS}.
...
minimizes a tradeoff between a data fitting term and a regularization term
...
regularization.} parameter. We recall that the quantity
\begin{dmath*}
    \mathfrak{R}_{\text{emp}}(f,
    \seq{s})=\frac{1}{N}\displaystyle\sum_{i=1}^NL(x_i, f, y_i)
...
Mazur and Schauder proposed in 1936 showing that \cref{eq:learning_rkhs} --and
\cref{eq:learning_rkhs_gen}-- attains a unique mimimizer
(see~\citet{kurdila2006convex, gorniewicz1999topological}).
\begin{theorem}[Mazur-Schauder]
    \label{cor:unique_minimizer}
    Let $\mathcal{H}$ be a Hilbert space and $\mathfrak{R}:\mathcal{H}\to
    \overline{\mathbb{R}}$ be a proper, convex, lower semi-continuous and
    coercive function. Then $\mathfrak{R}$ is bounded from below and attains a
    minimizer.  Moreover if $\mathfrak{R}$ is strictly convex the minimizer is
    unique.
\end{theorem}
This is easily verified for Ridge regression. Define
\begin{dmath} \label{eq:ridge}
...
$\mathfrak{R}_\lambda$ is continuous\mpar{Reminder, if $f\in\mathcal{H}_k,
...
$f_{\seq{s}} = \argmin_{f\in\mathcal{H}_K}\mathfrak{R}_\lambda(f)$ exists, is
unique and attained.
...
dimensional \acs{vv-RKHS} by a finite number of parameters $(u_i)_{i=1}^N$,
$u_i\in\mathcal{Y}$.
\paragraph{}
...
\begin{theorem}[Representer theorem]
    ...
    corresponding \acs{vv-RKHS}.
    \paragraph{}
    Let $c:\mathcal{Y}\times\mathcal{Y}\to\overline{\mathbb{R}}$ be a cost
    function such that $L(x, f, y)=c(f(x), y)$ is a proper convex lower
    ...
    hyperparameter. The solution $f_{\seq{s}}\in\mathcal{H}_K$ of the
    ...
    \begin{dmath}
        ...
        y_i\right) + \frac{\lambda}{2}\sum_{j,k=1}^{N}u_j^\adjoint
        K(x_j,x_k)u_k.
    \end{dmath}
\end{theorem}
...
\begin{proof}
    ...
    \end{dmath*}
    To ensure that $\mathfrak{R}_{\lambda}$ has a global minimizer we need the following
    ...
    \begin{lemma}
        \label{lm:strongly_convex_is_coercive} Let $\mathfrak{R}$ be a proper, convex,
        lower semi-continuous functional, defined on a Hilbert space
        $\mathcal{H}$. If $\mathfrak{R}$ is strongly convex, then $\mathfrak{R}$ is coercive.
    \end{lemma}
    \begin{proof}
    ...
    \begin{dmath*}
        ...
        \underbrace{\sum_{i=1}^{N}K_{x_i} y_i}_{\in\mathcal{H}_{K,
    ...
    \norm{f^{\parallel}}_{\mathcal{H}_K}^2$, \cref{eq:null1} shows that if
    $\lambda > 0$, clearly then
    \begin{dmath*}
        \mathfrak{R}_{\lambda}(f, \seq{s}) =
    ...
    $\mathcal{H}_{K, \seq{s}}$.
\end{proof}
The representer theorem shows that minimizing a functional in a \acs{vv-RKHS}
...
time $P$, to compute $K(x_i, x)u_i$, making a prediction using the
representer theorem takes $NP$. If $\mathcal{Y}=\mathbb{R}^p$, we can view
$K(x_i, x)$ as a matrix and then $P=O_t(p^2)$ thus making a prediction cost
$O_t(Np^2)$ operations.
...
\cref{eq:argmin_RKHS_theta}, then $f_{\seq{s}}=\Phi(\cdot)^\adjoint
...
focus on the representer theorem and the fact that there exist a partial
isometry $W$ between the \acs{vv-RKHS} and a feature space $\mathcal{H}$. We
show that if $\theta_{\seq{s}}$ is a solution of $\cref{eq:argmin_RKHS_theta}$,
...
the same than the solution of \cref{eq:argmin_applied} up to a linear
transformation. Namely, if $u_{\seq{s}}$ is the solution of \cref{eq:argmin_u},
$\theta_{\seq{s}}$ is the solution of \cref{eq:argmin_applied} and $\lambda>0$
we have
\begin{dmath*}
...
\paragraph{}
In \cref{fig:representer}, we generated the data from a sine wave to which we
...
In order to find a solution to \cref{eq:argmin_applied}, we first turn our
attention to gradient descent methods. In the following we let
\begin{dmath*}
    \mathfrak{R}_{\lambda}(\theta) =
    \frac{1}{N}\sum_{i=1}^Nc\left(\tildePhi{\omega}(x_i)^\adjoint \theta,
    y_i\right) + \frac{\lambda}{2}\norm{\theta}^2_{\tildeH{\omega}}
\end{dmath*}
...
Since the solution of \cref{eq:argmin_applied} is
unique when $\lambda>0$, a sufficient and necessary condition is that the
gradient of $\mathfrak{R}_{\lambda}$ at the minimizer $\theta_{\seq{s}}$ is zero. We use
...
$\theta_0\in \mathcal{H}_0$ if there exists a bounded linear operator
...
\begin{dmath*}
    ...
    \nabla_{f(\theta)}g(f(\theta)).
\end{dmath*}
Thus by linearity and applying the chaine rule to \cref{eq:argmin_applied} we
have
\begin{dgroup*}
    \begin{dmath*}
...
and $y_i\in\mathcal{Y}$ we have $\nabla_{\theta} \mathfrak{R}_{\lambda}(\theta) \in
\tildeH{\omega}$ and
\begin{dmath}
    \label{eq:grad_final}
    \nabla_{\theta} \mathfrak{R}_{\lambda}(\theta) = \frac{1}{N}\sum_{i=1}^N
...
\begin{example}[Naive closed form for the squared error cost]
    ...
    $\nabla_{\theta_{\seq{s}}} \mathfrak{R}_{\lambda}(\theta_{\seq{s}}) = 0$ we have
    \begin{dmath*}
        \frac{1}{N}\sum_{i=1}^N
...
\begin{remark}
    ...
    $\nabla_{\theta} \mathfrak{R}_{\lambda}(\theta)$, $V^*$,
    $\tildePhi{\omega}(x_i)^*$'s depends on the inner product of the respective
    spaces in which they are defined. Namely $\mathcal{Y}$, and
    $\tildeH{\omega}$. For instance if one chooses
    ...
    \Sigma^{-1}\tildePhi{\omega}(x)^\transpose$ and then \cref{eq:iff_solution}
    reads
    \begin{dmath*}
...
\afterpage{%
\begin{center}
    \begin{algorithm2e}[htb]\label{alg:close_form}
        \SetAlgoLined
        \Input{\begin{itemize}
            \item $\seq{s}=(x_i,
        ...
        \end{dmath*} %
        such that $\theta_{\seq{s}}$ minimizes \cref{eq:argmin_applied}, where
...
Hence \cref{alg:close_form} is better than its kernelized counterpart when
...
points $N$, thus it is more suitable for large scale learning provided that $D$
...
$r=2Dp$. This means that learning a vector-valued function has increased the
...
When developing \cref{alg:close_form} we considered that the feature map
...
$O_t(r^2p)$.  While this holds true in the most generic senario, in many cases
...
\subsection{Case of study: the decosubmposable kernel}
...
implementation evaluating a matrix vector product such as
...
\end{dmath*}
with this trick, many authors \citep{Sindhwani2013, brault2016random,
rosasco2010learning}
...
\begin{dmath*}
    ...
    \hiderel{-} Y \hiderel{=} 0.
\end{dmath*}
Many solvers exist to solve efficiently this kind of systems\mpar{See for
...
iterative solvers (or matrix-free solvers) are designed to solve any system of
equation of the form $PX=C$, where $P$ is a linear operator (not a matrix).
This is exactly our case where $\tildephi{\omega}(x)\otimes B^T$ is the matrix
...
Decomposable \acs{ORFF} \say{operator} to be fed to a matrix-free solvers.
\begin{pyblock}[efficient_linop][fontsize=\scriptsize]
...
\theta$ and $\tildePhi{\omega}(x)y$ falls down to $O_t(Dp'+p'p)$ and the same
holds for space complexity.
\begin{table}
    \centering
    \caption[Time complexity of efficient linear-operators for different
    \acs{ORFF}.]{Time complexity of efficient linear-operator (in matrix form)
    for different Feature maps given in \cref{table:efficient-op}.
...
illustrate the behavior of \acs{ORFF}-regression. First we study how well the
\acs{ORFF} regression recover the result of operator-valued kernel regression.
Second we show the advantages of \acs{ORFF} regression over independent
\acs{RFF} regression. A code implementing \acs{ORFF} is available in Operalib,
a framework for machine learning with \acsp{OVK}, at
...
\citep[section 5.1]{brault2016random}, we use an off-the-shelf
...
grid. Here we optimize \cref{eq:argmin_applied} using Scipy's \acs{L-BFGS-B}
...
row) reports the $R^2$ score on the test set versus the number of
...
On the second dataset, \cref{fig:curl_experiment} (top row) compares $R^2$
score and time of \acs{ORFF} regression using the trivial identity decomposable
...
repeated the experiments $80$ times to avoid randomness. We choose
$D=\frac{\max(N, 500)}{2}$ features, and optimized the problem with a second
order batch gradient.
\begin{table}[!h]
    \centering
...
%------------------------------------------------------------------------------
\section{Conclusion}
\acs{ORFF} approximations open the door to the literature of efficient learning
with linear models: the feature map can be seen as a function that linearize
non linear functions by embedding them in a high dimensional feature space in
which we can learn with linear models. Indeed, we described in this chapter how
learning a non linear, non parametric model with \acsp{OVK} is converted into
learning a linear and parametric model based on \acs{ORFF}.
