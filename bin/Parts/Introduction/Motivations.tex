...
space}.  In this chapter, $\mathcal{Y} \subset \mathbb{R}$. When
$\mathcal{Y}=\{1, \ldots, C:\}$, we talk about \emph{supervised
...
\end{figure}
The idea of kernel methods \citep{KIMELDORF1971,boser1992training} is to
consider a subset of the set of all functions, namely a Reproducing Kernel
Hilbert Space (\acl{RKHS}), associated to a well chosen positive definite and
symmetric function (\emph{a kernel}).
...
\begin{proposition}[\acs{SPSD} matrix]
    ...
    \begin{itemize}
        ...
        \item for any colunn vector $c= (c_1, \ldots, c_N)^\transpose \in
        \mathcal{M}_{1, N}(\mathbb{R})$, 
        \begin{dmath*}
...
\subsection{Random Fourier Features, Nystr\"om method and others}
