...
$K(x_i, x)$ as a matrix and then $P=O(p^2)$ thus making a prediction cost
$O(Np^2)$ operations.
...
\begin{dmath}
    \label{eq:cost_functional} \mathfrak{R}_{\lambda}(\theta) =
    \frac{1}{N}\sum_{i=1}^Nc\left(\tildePhi{\omega}(x_i)^\adjoint \theta,
    y_i\right) + \frac{\lambda}{2}\norm{\theta}^2_{\tildeH{\omega}}
\end{dmath}
...
\begin{center}
    \begin{algorithm2e}[H]
        ...
        \end{dmath*} %
        such that $\theta_{\seq{s}}$ minimizes \cref{eq:cost_functional}, where
