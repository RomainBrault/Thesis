...
\end{equation}
is an approximated feature map\mpar{\Ie~it satisfies $\tildePhi{1:D}(x)^\adjoint \tildePhi{1:D}(z)\converges{\asurely}{D\to\infty}K(x,z)$ where $K$ is a $\mathcal{Y}$-Mercer \acs{OVK}.} of a shift-invariant $\mathcal{Y}$-Mercer \acl{OVK}.
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof} Apply \cref{pr:feature_operator} to $\widetilde{\Phi}$ to show that $\widetilde{K}$ is an \acs{OVK}. \Cref{pr:kernel_signature} shows that $\widetilde{K}$ is shift-invariant since $\widetilde{K}(x,z)=\widetilde{K}_e\left(x,\groupop z^{-1}\right)$. Note that $\widetilde{K}$ is not sensus stricto a $\mathcal{Y}$-Mercer kernel since it is a random variable. However $\lim_{D\to\infty} \widetilde{K} = \expectation \widetilde{K} = K$ is $\mathcal{Y}$-Mercer.
\end{proof}
...
\begin{definition}[Random Fourier feature operator] Let
\begin{dmath*}
...
\begin{dmath*}
(\tildeW{D} \theta)(x) \colonequals \tildePhi{1:D}(x)^\adjoint \theta =\frac{1}{D}\sum_{j=1}^D \conj{\pairing{x,\omega_j}}B(\omega_j)g(\omega_j), \enskip \omega_j \hiderel{\sim} \probability_{\dual{\Haar},\rho} \enskip\text{\iid}.
\end{dmath*}
...
% \end{sideways}
\caption[Relationships between feature-maps.]{Relationships between feature-maps. $\widetilde{\mathcal{H}}^D = \frac{1}{\sqrt{D}}\Set{\Vect_{j=1}^Dg(\omega_j)| \forall g\in L^2\left(\dual{\mathcal{X}}, \probability_{\dual{\Haar},\rho}, \mathcal{Y}'\right), \enskip \omega_j\sim \probability_{\dual{\Haar},\rho} \enskip \text{\iid} }$.}
...
In the following, $K(x,z)=K_{1-c}(x\odot z^{-1})$ is a $\mathcal{Y}$-Mercer matrix-valued kernel on $\mathcal{X}=(-c;+\infty)^d$ invariant \wrt~the group operation\mpar{The group operation $\odot$ is defined in \cref{subsubsec:skewedchi2}.} $\odot$. Then the function $\tildePhi{1:D}$ defined as follow is an \acl{ORFF} of $K_{1-c}$.
\begin{dmath*}
...
\begin{algorithm2e}[H]\label{alg:ORFF2_construction}
    ...
    \end{itemize}}
    \Output{A random feature $\tildePhi{1:D}(x)$ such that $\tildePhi{1:D}(x)^\adjoint \tildePhi{1:D}(z) \approx K_e(x\groupop z^{-1})$.}
    \BlankLine
    Draw $D$ random vectors $(\omega_j)_{j=1}^D$ \iid~from the probability law $\probability_{\dual{\Haar},\rho}$\;
    \Return $\tildePhi{1:D}(x)=\frac{1}{\sqrt{D}}\Vect_{j=1}^D\pairing{x, \omega_j}B(\omega_j)^\adjoint $\;
...
$\tildef{1:D}(x)=\tildePhi{1:D}(x)^\adjoint \theta$, where $\theta$ lives in some redescription space\mpar{If $\mathcal{Y}=\mathbb{R}$ then $\mathcal{Y}'=\mathbb{R}$ thus $\theta=\mathbb{R}^D$ endowed with the euclidean inner product.} $\widetilde{\mathcal{H}}^D$. We are interested in finding a parameter vector $\theta_*$ such that
\begin{dmath}
...
\end{dmath}
has the form $\widetilde{f}_*=\tildePhi{1:D}(\cdot)^\adjoint\theta_*$, where $\theta_*\in (\Ker \widetilde{W})^{\perp}$ and
\begin{dmath}
\theta_* = \argmin_{\theta'\in\Vect_{j=1}^D\mathcal{Y}'} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x_i)^\adjoint \theta' - y_i}^2_{\mathcal{Y}} + \frac{\lambda}{2}\norm{\theta'}^2_{\Vect_{j=1}^D\mathcal{Y}'}.
\end{dmath}
\end{corollary}
\begin{proof}
Since $\widetilde{K}$ is an operator-valued kernel, \cref{eq:argmin_RKHS_rand} has a solution of the form
\begin{dmath*}
\widetilde{f}_* = \sum_{i=1}^N \widetilde{K}(\cdot, x_i)u_i, \qquad u_i \hiderel{\in} \mathcal{Y}', \enskip x_i \hiderel{\in}\mathcal{X}
= \sum_{i=1}^N \tildePhi{1:D}(\cdot)^\adjoint \tildePhi{1:D}(x_i)u_i
\hiderel{=} \tildePhi{1:D}(\cdot)^\adjoint \underbrace{\left(\sum_{i=1}^N\tildePhi{1:D}(x_i)u_i\right)}_{=\theta\in \left(\Ker \widetilde{W}\right)^\perp\subset \widetilde{\mathcal{H}}^D}.
\end{dmath*}
Let
\begin{dmath*}
\theta_* = \argmin_{\theta\in\left(\Ker \widetilde{W}\right)^\perp} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x)^\adjoint\theta - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\tildePhi{1:D}(\cdot)^\adjoint\theta}^2_{\widetilde{K}}
\end{dmath*}
Since $\theta\in(\Ker W)^\perp$ and $W$ is an isometry from $(\Ker W)^\perp\subset \widetilde{\mathcal{H}}^D$ onto $\mathcal{H}_{\widetilde{K}}$, we have $\norm{\tildePhi{1:D}(\cdot)^\adjoint\theta}^2_{\widetilde{K}} = \norm{\theta}^2_{\widetilde{\mathcal{H}}^D}$. Hence
\begin{dmath*}
\theta_* = \argmin_{\theta\in\left(\Ker \widetilde{W}\right)^\perp} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x)^\adjoint\theta - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta}^2_{\widetilde{\mathcal{H}}^D},
\end{dmath*}
Since W is bounded, $\Ker \widetilde{W}$ is closed, so that we can perform the decomposition $\widetilde{\mathcal{H}}^D=\left(\Ker \widetilde{W}\right)^\perp\oplus \left(\Ker \widetilde{W}\right)$. Then clearly by linearity of $W$ and the fact that for all $\theta^{\parallel}\in\Ker \widetilde{W}$, $\widetilde{W}\theta^{\parallel}=0$, if $\lambda > 0$ we have
\begin{dmath*}
\theta_* = \argmin_{\theta\in\widetilde{\mathcal{H}}^D} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x)^\adjoint\theta - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta}^2_{\widetilde{\mathcal{H}}^D}
= \argmin_{\theta\in\left(\left(\Ker \widetilde{W}\right)^\perp\oplus \Ker \widetilde{W}\right)} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{(\widetilde{W}\theta)(x) - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta}^2_{\widetilde{\mathcal{H}}^D}
= \argmin_{\substack{\theta^{\perp}\in\left(\Ker \widetilde{W}\right)^\perp, \\ \theta^{\parallel}\in\Ker \widetilde{W}}} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\left(\widetilde{W}\theta^{\perp}\right)(x) + \underbrace{\left(\widetilde{W}\theta^{\parallel}\right)}_{=0 \enskip\text{for all}\enskip \theta^{\parallel}}(x) - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta^\perp}^2_{\widetilde{\mathcal{H}}^D} + \underbrace{\frac{\lambda}{2}\norm{\theta^{\parallel}}^2_{\widetilde{\mathcal{H}}^D}}_{=0 \enskip\text{if}\enskip \theta^{\parallel}=0}
= \argmin_{\theta^{\perp}\in\left(\Ker \widetilde{W}\right)^\perp} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x)^\adjoint\theta^{\perp} - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta^{\perp}}^2_{\widetilde{\mathcal{H}}^D}.
\end{dmath*}
Eventually for all $g\in L^{2}\left(\dual{\mathcal{X}},\probability_{\dual{\Haar},\rho},\mathcal{Y}'\right)$ and $\omega_j \sim \probability_{\dual{\Haar},\rho}$ \iid,
\begin{dmath*}
\theta_* = \argmin_{\theta\in\widetilde{\mathcal{H}}^D} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x_i)^\adjoint\theta - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{\theta}^2_{\widetilde{\mathcal{H}}^D}
= \argmin_{\frac{1}{\sqrt{D}}\Vect_{j=1}^Dg(\omega_j)} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\frac{1}{D}\sum_{j=1}^D\conj{\pairing{x_i,\omega_j}}B(\omega_j)g(\omega_j) - y_i}^2_{\mathcal{Y}}+ \frac{\lambda}{2}\frac{1}{D}\sum_{j=1}^D\norm{g(\omega_j)}^2_{\mathcal{Y'}}
= \argmin_{\theta'\in\Vect_{j=1}^D\mathcal{Y}'} \frac{1}{2N}\displaystyle\sum_{i=1}^N\norm{\tildePhi{1:D}(x_i)^\adjoint \theta' - y_i}^2_{\mathcal{Y}}+ \frac{\lambda}{2}\norm{\theta'}^2_{\Vect_{j=1}^D\mathcal{Y}'}. \qquad \qed
\end{dmath*}
\end{proof}
% Since $\theta\in\widetilde{\mathcal{H}}^D\left(\mathcal{Y}'\right)$ lives in a different space than $f\in\mathcal{H}_K$ is makes no sense to show the convergence of $\theta_*$ to $f_*$. However we can verify that $\tildef{1:D}$ converges to $f\in\mathcal{H}_K$ pointwise. We first introduce the following lemma and assumptions. First we characterize extremum estimators. Let $\mathcal{H}$ be any Hilbert space.
% \begin{assumption}[EE]
% \label{ass:ee}
...
% \begin{proposition}[Consitensy of ORFF \wrt~\acs{vv-RKHS}] Let $K$ be a $\mathcal{Y}$-Mercer Kernel. If $\theta_*\in\widetilde{\mathcal{H}}^D\left(\mathcal{Y}'\right)$ is the minimizer of \cref{eq:argmin_applied} and $f_*\in\mathcal{H}_K$ is the minimizer of \cref{eq:argmin_RKHS}. Then
% \begin{dmath}
