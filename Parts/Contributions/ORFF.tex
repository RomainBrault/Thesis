...
Let $\theta=\vect_{j=1}^Dg(\omega_j)$ where $\omega_j \sim \mu$. We call random Fourier feature operator
\begin{dmath}
(\tildeW \theta)(x) \hiderel{\coloneqq} \tildePhi(x)^*\theta\hiderel{=}\frac{1}{D}\sum_{j=1}^D \conj{\pairing{x,\omega_j}}B(\omega_j)g(\omega_j), \qquad \omega_j \hiderel{\sim} \mu.
\end{dmath}
\end{definition}
