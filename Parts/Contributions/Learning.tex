...
\caption[\acs{ORFF} Representer theorem]{ORFF Representer theorem. We trained a first model named $\tildeK{\omega}$ following}
...
\end{cases}
\end{dmath*}
This equivalent formulation allows us to keep the notation \say{lighter}. With these notations, a summary of efficient linear operators in matrix form is given in \cref{tb:efficient-op} and \cref{tb:efficient2-op}.
\afterpage{%
\begin{landscape}
\begin{table}[ht]
\centering
...
\midrule
    ...
    Gaussian \newline curl-free\tnote{2} & $\Theta^c, \Theta^s\mapsto $ & $\displaystyle\sum_{j=1}^D\omega_j\omega_j^T\left(U(\tildephi{\omega}^{c}(X)_{j\bullet})^T\tildephi{\omega}^{c}(X)_{j\bullet} + U(\tildephi{\omega}^{s}(X)_{j\bullet})^T\tildephi{\omega}^{s}(X)_{j\bullet} \right)$ & $U\mapsto$ & $\begin{cases}\Theta_j^c=\omega_j^T\displaystyle\sum_{i=1}^N \tildephi{\omega}^{c}(X)_{ji}U_{\bullet i} \\ \Theta_j^s=\omega_j^T\displaystyle\sum_{i=1}^N \tildephi{\omega}^{s}(X)_{ji}U_{\bullet i}\end{cases}$ \\
...
% \toprule
%     Kernel & $\tildePhi{\omega}(X)\tildePhi{\omega}(X)^T:\Theta\mapsto$ & $\tildePhi{\omega}(X)^T\tildePhi{\omega}(X):Y\mapsto$ \\
% \midrule
%     Decomposable\tnote{1} & $\Theta'=B^TB\Theta \tildephi{\omega}(X)\tildephi{\omega}(X)^T$ & $Y'=BB^TY\tildephi{\omega}(X)^T\tildephi{\omega}(X)$ \\

%     Gaussian curl-free\tnote{2} & $\Theta'=\displaystyle\sum_{j=1}^D\left(\tildephi{\omega}^{c}(X)\tildephi{\omega}^{c}(X)^T\right)_{\bullet j}\Theta^{c}_j\norm{\omega_j}_2^2$ & $\begin{pmatrix}\omega_j^T\displaystyle\sum_{i=1}^N \tildephi{\omega}^{c}(X)_{ji}Y_{\bullet i} \\ \omega_j^T\displaystyle\sum_{i=1}^N \tildephi{\omega}^{s}(X)_{ji}Y_{\bullet i} \end{pmatrix}_{j=1}^D$ \\

%     Gaussian divergence-free\tnote{2,3} & $\displaystyle\sum_{j=1}^D\left(\norm{\omega_j}_2I_d-\omega_j\omega_j^T\right)\Theta_{j\bullet}\left( \tildephi{\omega}^{c}(X)_{j\bullet} + \tildephi{\omega}^{s}(X)_{j\bullet} \right)$ & $\begin{pmatrix}\left(\norm{\omega_j}_2I_d-\omega_j\omega_j^T\right)\displaystyle\sum_{i=1}^N \tildephi{\omega}^{c}(X)_{ji}Y_{\bullet i} \\ \left(\norm{\omega_j}_2I_d-\omega_j\omega_j^T\right)\displaystyle\sum_{i=1}^N \tildephi{\omega}^{s}(X)_{ji}Y_{\bullet i} \end{pmatrix}_{j=1}^D$ \\
% \bottomrule
% \end{tabularx}
% \begin{tablenotes}
% \item[1] Where $\tildephi{\omega}(X)=\begin{pmatrix} \tildephi{\omega}(X_{\bullet 1}) & \hdots & \tildephi{\omega}(X_{\bullet N}) \end{pmatrix}\in\mathcal{M}_{r, N}$ is any design matrix, with scalar feature map $\tildephi{\omega}:\mathbb{R}^d\to\mathbb{R}^r$ such that $\tildephi{\omega}(x)^\adjoint \tildephi{\omega}(z)=k(x,z)\in\mathbb{R}$ for all $x$, $z\in\mathcal{X}$. The input data $X\in\mathcal{M}_{d,N}(\mathbb{R})$, the output data $Y\in\mathcal{M}_{u,N}(\mathbb{R})$, the parameter matrices $\Theta^c$ and $\Theta^s\in\mathcal{M}_{u', r}(\mathbb{R})$ and the decomposable operator $B\in\mathcal{M}_{u,u'}(\mathbb{R})$.
% \item[2] Where $\tildephi{\omega}^{c}(X)_{ji}=\cos\inner{\omega_j, x_i}$ and $\tildephi{\omega}^{s}(X)_{ji}=\sin\inner{\omega_j, x_i}$, $j\in\mathbb{N}_D$ and $i\in\mathbb{N}_N$. Thus $\tildephi{\omega}^{c}(X)\in\mathcal{M}_{D,d}(\mathbb{R})$ and $\tildephi{\omega}^{s}(X)\in\mathcal{M}_{D,d}(\mathbb{R})$. The input data $X\in\mathcal{M}_{d,N}(\mathbb{R})$, the output data $Y\in\mathcal{M}_{d,N}(\mathbb{R})$ (here $u=d$), the parameter matrices $\Theta^c$ and $\Theta^s\in\mathbb{R}^D$, $\omega_j\sim \mathcal{N}(0,\sigma^{-2}I_d)$ \iid~for all $j\in\mathbb{N}_D$.
% \item[3] Here, $\Theta^c$ and $\Theta^s\in\mathcal{M}_{d,D}(\mathbb{R})$.
% \end{tablenotes}
% \end{threeparttable}
% \label{tb:efficient2-op}
