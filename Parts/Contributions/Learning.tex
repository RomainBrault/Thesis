...
\end{dmath*}
If $f:\mathcal{H}\to\mathbb{R}$, $f\in\mathcal{H}^\adjoint$ and by Riesz's representation theorem we define the gradient of $f$ noted $\nabla_{\theta} f(\theta)$ as the the vector in $\mathcal{H}$ such that
\begin{dmath*}
\inner{\nabla_{\theta} f(\theta), h}_{\mathcal{H}} = \derivative{f(\theta)}{\theta} \circ h.
\end{dmath*}
For a function $f:\mathcal{H}_0\to\mathcal{H}_1$ the jacobian of $f$ as
\begin{dmath*}
\jacobian_{\theta} f(\theta) = \derivative{f(\theta)}{\theta}\right.
\end{dmath*}
