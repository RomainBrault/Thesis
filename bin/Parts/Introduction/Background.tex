...
\paragraph{}
If $(\mathcal{X},\mathcal{T}_x)$ is a topological space and $x$ is a point in
...
second countable. Moreover every second countable locally compact Hausdorff
...
$(\mathcal{X},\mathcal{B}(\mathcal{X}))$ is a map $\mu:
\mathcal{B}(\mathcal{X}) \to \overline{\mathbb{R}}_+$ which is zero on the
empty set and countably additive, \acs{ie}~for any subset
$(\mathcal{Z}_n)_{n\in\mathbb{N}}$ is a sequence of pairwise disjoint
measurable sets,
\begin{dmath*}
...
coincides with that of boundedness. We recall that the norm of a linear
...
\begin{table}
    ...
    \begin{tabularx}{\textwidth}{cX}
        ...
        \endhead
            ...
            $\overline{\mathbb{R}}$ & The one point compacification of the real
...
thesis we focus on positive-definite functions. Let $\mathcal{Y}$ be a complex
...
are invariant under translation. There exists, up to a positive multiplicative
...
\begin{definition}[The Haar measure]
    ...
    \begin{propenum}
        \item if $\mathcal{Z} \subseteq \mathcal{X}$ is open and not empty,
        then $\Haar(\mathcal{Z})>0$.
...
space $(\mathcal{X},\mathcal{B}(\mathcal{X}),\Haar)$. In other words the
(topological) space $\mathcal{X}$ endowed with its Borel $\sigma$-algebra
$\mathcal{B}(\mathcal{X})$ and a measure $\Haar$.  If $\Haar(\mathcal{X})=1$
then the space $(\mathcal{X},\mathcal{B}(\mathcal{X}),\Haar)$ is called a
probability space.  Last but not least, on the additive group $(\mathbb{R},+)$,
the Lebesgue measure noted $\Leb$ is a valid Haar measure. For a concise
introduction and important properties we refer the reader to the lecture of
...
an \ac{LCA} group is an \ac{LCA} group such that we can endow
...
\begin{example}[\citet{li2010random}]
    ...
    $\Haar(\mathcal{Z})=\int_{\mathcal{Z}}(z+c)^{-1}d\Leb(z)$.
\end{example}
\begin{proof}
...
\begin{proposition}[\citet{folland1994course}]
...
\begin{theorem}[Fourier inversion (\citet{folland1994course})]
...
and $\omega\in\dual{\mathcal{X}}$.  If one endows $\mathcal{X}$ with the
...
\end{dmath*}
in order to have $\mathcal{F}^{-1}\FT{f}=f$. If one usess the cleaner equivalent
...
and makes the Haar measure self-dual. However it is of lesser use in practice
...
\end{dmath*}
We conclude this subsection by recalling the injectivity property of the
\acl{FT}.
\begin{corollary}[\acl{FT} injectivity (\citet{folland1994course})]
...
$x\in\mathcal{X}$. \acs{ie}~for all $\mathcal{U}\subset\mathcal{H}$,
$\mathcal{U}\neq \Set{0}$,
\begin{dmath*}
...
function $f$ belonging to a class (space) of functions
$\mathcal{F}(\mathcal{X};\mathcal{Y})$ that minimizes a criterion called the
true risk (see \cref{sec:about_statistical_learning}). The class of functions we
consider are functions living in a Hilbert space
...
\begin{definition}[\acl{vv-RKHS}~\citep{carmeli2006vector,Micchelli2005}]
    ...
    \begin{propenum}
        ...
        \mathcal{Y})$);
        \item for all $x\in\mathcal{X}$, there exists a positive constant
        $C_x\in\mathbb{R}$ such that for all $f\in\mathcal{H}$
        \begin{dmath}
...
\acl{vv-RKHS}.
\begin{proposition}[\citet{carmeli2006vector}]
...
\begin{enumerate}
    ...
    $x\in\mathcal{X}$,
    $\text{ev}_x^\adjoint y=K_xy=K(\cdot,x)y$ such that
    \begin{dmath}
...
\begin{proof}
    ...
    definition such that the evaluation map
    \begin{dmath*}
        \text{ev}_x=(W\Phi(x))^\adjoint
    \end{dmath*}
    is bounded, thus continuous. Then the reproducing kernel is given for all
    $x$, $z\in\mathcal{X}$ by
    \begin{dmath*}
...
functions $g\in\mathcal{H}$ such that $Wg=f$, the only one making the norm in
the \ac{vv-RKHS} and the redescription space is the one with minimal norm.
\paragraph{}
In this work we mainly focus on the class of kernels inducing a \ac{vv-RKHS} of
...
\end{definition}
\paragraph{}
The following proposition characterizes $\mathcal{Y}$-Mercer kernel in terms of
...
\begin{proof}
    ...
    f}_{\mathcal{Y}}$=$\norm{f(x)}_{\mathcal{Y}}\le \norm{f}_{\infty}$. Hence
    there exists a constant $M\in\mathbb{R}_+$ such that for all
    $x\in\mathcal{X}$, $\norm{K_x}_{\mathcal{Y}, K}\le M$.
    Therefore from \cref{eq:kernel_operator_product}, for all
    $x\in\mathcal{X}$, $\norm{K(x, x)}_{\mathcal{Y},
    \mathcal{Y}}=\norm{K_x^\adjoint}_{K, \mathcal{Y}}^2\le M^2$.
    Conversely assume that the function $x\mapsto\norm{K(x,x)}_{\mathcal{Y},
    \mathcal{Y}}$ is locally bounded and
    $K_xy\in\mathcal{C}(\mathcal{X};\mathcal{Y})$. For all $f\in\mathcal{H}_K$
    and all $x\in\mathcal{X}$,
    \begin{dmath*}
        \norm{f(x)}_{\mathcal{Y}} \hiderel{=}
        \norm{f}_K\sqrt{\norm{K(x,x)}_{\mathcal{Y}, \mathcal{Y}}}\hiderel{\le}
        M \norm{f}_K.
    \end{dmath*}
...
\begin{lemma}[Separable \ac{vv-RKHS}~(\citet{carmeli2006vector})]
...
\end{lemma}
\begin{proof}
    The separability of $\mathcal{X}$ ensure that there exist a countable dense
    ...
    in $\mathcal{X}$, for all $x\in\mathcal{X}$, $f(x)=0$ thus $f=0$.
\end{proof}
...
\end{dmath*}
A group representation $\lambda_z$ describes the group by making it act on a
...
\begin{proposition}[Kernel signature~(\citet{Carmeli2010})]
    ...
    \begin{dgroup}
        \begin{dmath}
            K(x,z)=K_e^\adjoint\lambda_{\inv{x}\groupop z}K_e \condition{%
            $\forall (x,z)\hiderel{\in}\mathcal{X}^2$},
        \end{dmath}
        \begin{dmath}
            \label{pr:kernel_signature_4} \norm{K(x,x)}_{\mathcal{Y},
            \mathcal{Y}}=\norm{K_e(e)}_{\mathcal{Y}, \mathcal{Y}}
            \condition{$\forall x\hiderel{\in}\mathcal{X}$}.
        \end{dmath}
    \end{dgroup}
\end{proposition}
\begin{proof}
    ...
    \begin{dmath*}
        ...
        t)y,y'}_{\mathcal{Y}, \mathcal{Y}}=\inner{K(t',t)y, y'}_{\mathcal{Y},
        \mathcal{Y}}\hiderel{=}\inner{K_ty, K_{t'}y'}_K
    \end{dmath*}
...
defined and refer the interrested reader to the respective authors original
work.
\paragraph{}
...
\begin{proposition}[Decomposable kernel~(\citet{Micchelli2005})]
    \label{dec-kernel}
...
(tasks).  When no prior knowledge is available, $\Gamma$ can be learned with
one of the algorithms proposed in the literature~\citep{Dinuzzo2011,
Sindhwani2013, Lim2015}. Another interesting property of the decomposable
kernel is its universality (a kernel which may approximate an arbitrary
continuous target function uniformly on any compact subset of the input space).
A reproducing kernel $K$ is said \emph{universal} if the associated
\ac{vv-RKHS} $\mathcal{H}_K$ is \emph{dense} in the space of continuous
functions $\mathcal{C}(\mathcal{X},\mathcal{Y})$.  The conditions for a kernel
to be universal have been discussed in~\citet{caponnetto2008,Carmeli2010}. In
...
$\Gamma$ acts as a regularizer between the components of the outputs $f_k =
\inner{f(\cdot), e_k}_{\mathcal{Y}}$ of a function $f\in\mathcal{H}_K$.
\begin{proposition}[Kernels and Regularizers~(\citet{Alvarez2012})]
    \label{pr:kernel_reg}
    ...
    \end{dmath}
    where $f_i=\inner{f(\cdot),e_i}_{\mathcal{Y}}$ 
    (resp $f_j=\inner{f(\cdot),e_j}_{\mathcal{Y}}$), denotes the $i$-th 
    (resp $j$-th) component of $f(\cdot)$.
\end{proposition}
...
\end{figure}
\begin{proposition}[Curl-free and Div-free kernel~(\citet{Macedo2008})]
    \label{curl-div-free}
...
Baldassare2012}. The next class of kernels we present are transformable kernels,
whose action on each coordinate of an output vector is determined by
\say{views} of an input data.
\begin{proposition}[Transformable kernel~(\citet{caponnetto2008})]
...
\end{proposition}
Transformable kernels have been successfully used for network inference from
time series by means of autoregressive models (\citet{lim2013okvar,
...
\begin{proposition}[Hilbert Schmidt Integral
kernel~(\citet{kadri2015operator})]
    \label{def:hilbert_schmidt_integral_kernel}
...
\begin{dmath}
    ...
    t)g(t)d\mu(t).
\end{dmath}
In \citet{kadri2015operator}, the authors studied the case where
...
\subsection{Some use of Operator-valued kernels}
...
\paragraph{}
Multi-task regression \citep{micchelli2004kernels}  and structured multi-class
