...
Let $g\in \mathcal{H}=L^2(\mathcal{\dual{X}},d\mu;\mathcal{Y}')$ and let $\theta \colonequals \vect_{j=1}^Dg(\omega_j)$ where $\omega_j\sim \mu$. Then for all $g\in\mathcal{H}$,
\begin{propenum}
...
Given a local loss function $L: \mathcal{X}\times\mathcal{F}\times\mathcal{Y}\to \mathbb{R}^+$, we are interested in finding a \emph{vector-valued function} $f_*:\mathcal{X}\to\mathcal{Y}$, that lives in a RKHS and minimize a tradeoff between a data fitting term $L$ and a regularization term to prevent from overfitting. Namely finding $f_*\in\mathcal{H}_K$ such that
\begin{dmath*}
f_* = \argmin_{f\in\mathcal{H}_K}  \frac{1}{2N}\displaystyle\sum_{i=1}^NL(x_i, f, y_i) + \frac{\lambda}{2}\norm{f}^2_{\mathcal{H}_K},
\label{eq:learning_rkhs}
\end{dmath*}
where $\lambda\in\mathbb{R}_+$ is a regularization parameter. A common choice of data fitting term for regression is $L:x_i,\enskip f,\enskip y_i \mapsto \norm{f(x_i)-y_i}_{\mathcal{Y}}^2$. Regression in \acl{vv-RKHS} has been well studied and a cornerstone of learning in \acs{vv-RKHS} is the representer theorem\mpar{}.
...
\label{eq:learning_rkhs}
\end{dmath}
has the form $f_*=\sum_{i=1}^NK(x_i,\cdot)u_i$, where $u_i\in\mathcal{Y}$.
\end{theorem}
...
\begin{proof}
Let $J(f)=\sum_{i=1}^N\norm{f(x_i) - y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{f}^2_{\mathcal{H}_K}$ be the functional to minimize. We use the notion of Frechet derivative, noted $D_F$. This is the strongest notion of derivative in a normed vector space, see \citet{kurdila2006convex}.
...
\end{dmath*}
Let $G_i(f)=\norm{f(x_i)-y_i}_{\mathcal{Y}}^2$. Then $J(f)=\frac{1}{2N}\sum_{i=1}^NG_i(f)+\frac{\lambda}{2}\norm{f}_{\mathcal{H}_K}^2$. Moreover the directional Gateaux derivative obey $D_GJ(f, h)=\inner{D_GJ(f),h}_{\mathcal{H}_K}$. Thus
\begin{enumerate}
\item Regularization term:
\begin{dmath}
\label{enum:Gateaux_diff_reg}
\lim_{t\to 0}\frac{\norm{f+th}^2_{\mathcal{H}_K}-\norm{f}_{\mathcal{H}_K}^2}{t}=2\inner{f,h}{\scriptstyle\implies}D_G\norm{f}_{\mathcal{H}_K}^2\hiderel{=}2f.
\end{dmath}
\item Data fitting term:
\begin{dmath}
\label{enum:Gateaux_diff_fit}
\lim_{t\to 0}\frac{\norm{y_i-f(x_i)+th(x_i)}_{\mathcal{Y}}^2-\norm{y_i-f(x_i)}_{\mathcal{Y}}^2}{t} = -2\inner{y_i-f(x_i),h(x_i)}_{\mathcal{Y}} \hiderel{=} -2\inner{K_{x_i}(y_i-f(x_i)),h}_{\mathcal{H}_K}\\{\scriptstyle\implies}D_G G_i(f)\hiderel{=}-2K_{x_i}(y_i-f(x_i)).
\end{dmath}
\end{enumerate}
...
$\tildef{1:D}(x)=\tildePhi{1:D}(x)^*\theta$, where $\theta$ lives in some redescription space $\left(\mathcal{Y}'\right)^D$.
\begin{proposition}[Consitensy of ORFF \wrt~\acs{vv-RKHS}]
Let $K$ be a $\mathcal{Y}$-Mercer Kernel such that $\tildePhi{1:D}(x)^\adjoint\tildePhi{1:D}(z)\approx K(x,z)$. Moreover let
\begin{dmath*}
\label{eq:argmin_applied}
\theta_*=\argmin_{\theta\in\left(\mathcal{Y}'\right)^D} \frac{1}{2N}\sum_{i=1}^N\norm{\tildePhi{1:D}(x_i)^\adjoint \theta-y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2D}\norm{\theta}^2_{\left(\mathcal{Y}'\right)^D}
\end{dmath*}
and
\begin{dmath*}
\label{eq:argmin_RKHS}
f_*=\argmin_{f\in\mathcal{H}_K} \frac{1}{2N}\sum_{i=1}^N\norm{f(x_i)-y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{f}^2_{\mathcal{H}_K}
\end{dmath*}
Then $\tildePhi{1:D}(\cdot)^\adjoint\theta_* \converges{\proba}{D\to\infty}f_*\in\mathcal{H}_K$.
\end{proposition}
\begin{proof}
We have that for all $f\in\mathcal{H_K}$, there exists a $g\in\mathcal{H}$ such that $f(x)=(Wg)(x)=\Phi(x)^\adjoint g$ where $W$ is a partial isometry with initial subspace $(\Ker W)^\bot\subset\mathcal{H}$ and final subspace $\mathcal{H}_K$. Let
\begin{dmath}
\label{eq:argmin_HS}
g_*=\argmin_{g\in\mathcal{H}} \frac{1}{2N}\sum_{i=1}^N\norm{(Wg)(x_i)-y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{g}^2_{\mathcal{H}_K}
\end{dmath}
Since $W$ is a partial isometry and \cref{eq:argmin_RKHS} has a unique global optimizer, \cref{eq:argmin_HS} has a unique global optimizer $g_*$ such that $Wg_*=f_*$. Let
\begin{dmath}
Q_0(g)=\frac{1}{2N}\sum_{i=1}^N\norm{(Wg)(x_i)-y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2}\norm{g}^2_{\mathcal{H}_K},
\end{dmath}
and $\theta_g=\vect_{j=1}^Dg(\omega_j)$. We define the empirical estimator of $Q_0$ as
\begin{dmath}
Q_D(g)=\frac{1}{2N}\sum_{i=1}^N\norm{\tildePhi{1:D}(x_i)^\adjoint \theta_g-y_i}_{\mathcal{Y}}^2 + \frac{\lambda}{2D}\norm{\theta_g}^2_{(\mathcal{Y}')^D}
\end{dmath}
With these notatition we have $g_*=\argmin_{g\in\mathcal{H}} Q_0(g)$. Moreover we note $\tilde{g}_*=\argmin_{g\in\mathcal{H}} Q_D(g)$. Then
\begin{dmath}
