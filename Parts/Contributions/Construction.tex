...
Let $K_e$ be the signature of a shift-invariant $\mathcal{Y}$-Mercer kernel and let $\inner{y', C(\cdot)y}_{\mathcal{Y}}=\IFT{\inner{y', K_e(\cdot)y}_{\mathcal{Y}}}$ for all $y$. $y'\in\mathcal{Y}$. Then
\begin{propenum}
...
\begin{dmath*}
\inner{y', C(\omega)y}=\conj{\inner{y', C\left(\inv{\omega}\right)y}}\hiderel{=}\inner{C\left(\inv{\omega}\right)^* y', y}
=\inner{C\left(\omega\right) y', y}\hiderel{=}\inner{y', C\left(\omega\right)^\adjoint y}.
\end{dmath*}
...
\caption[Relationships between feature-maps.]{Relationships between feature-maps. For any realization of $\omega_j\sim\probability_{\dual{\Haar},\rho}$ \iid~, $\tildeH{\omega} = \Vect_{j=1}^D \mathcal{Y}'$.}
