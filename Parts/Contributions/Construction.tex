...
approximated feature map. In this work, we are interested in constructing an
approximated operator-valued feature map to learn vector-valued functions.
...
another proposition of~\citeauthor{carmeli2006vector} and showis that if the
...
obtained in \cref{pr:spectral} might not be bounded. The unboundedness of
$\norm{A(\cdot)}_{\mathcal{Y},\mathcal{Y}}$ forbids the use of the most simple
...
smola1998connection}. We also note that the regularization property in
$\mathcal{H}_K$ does not depends (as expected) on the decomposition of
$A(\omega)$ into $B(\omega)B(\omega)^\adjoint $. Therefore the decomposition
should be chosen such that it optimizes the computation cost. For instance if
