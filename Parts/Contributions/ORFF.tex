...
% The following lemma is a direct consequence of the definition of $C(\omega)$ as the \acl{FT} of the adjoint of $K_e$ and helps simplifying the definition of \acs{ORFF}.
% \begin{lemma}
% \label{lm:C_characterization}
% Let $K_e$ be the signature of a shift-invariant $\mathcal{Y}$-Mercer kernel and let $C(\omega)=\FT{K_e(\cdot)^\adjoint}(\omega)$. Then $C(\omega)$ is Hermitian and $C$ is even. As a consequence $K_e$ is even and

% \end{lemma}
% \begin{proof}
% Suppose that $\mathcal{Y}$ is a real Hilbert space. Then by definition for all $x$, $z\in\mathcal{X}$, $K(x,z)=K(z,x)=K(z,x)^\adjoint$ so that $K_e(-\delta)=K_e(\delta)=K_e(\delta)^\adjoint$. Taking the Fourier transform yields
% \begin{dmath*}
% \mathcal{F}\mathcal{R}\left[\inner{y,K_e(\cdot)y'}\right]\hiderel{=}\IFT{\inner{y,K_e(\cdot)y'}}\hiderel{=}\IFT{\inner{y,K_e(\cdot)^\adjoint}
% \end{dmath*}
% Hence using the fact that $\mathcal{F}^{-1}=\mathcal{F}\mathcal{R}$ and $\mathcal{R}K_e=K_e$ we have
% \begin{dmath*}
% \FT{\inner{y,K_e(\cdot)y'}}\hiderel{=}\IFT{\inner{y,K_e(\cdot)y'}}\hiderel{=}\IFT{\inner{y,K_e(\cdot)^\adjoint}y'}.
% \end{dmath*}
% Eventually we conclude $C(\omega)\hiderel{=}c(\omega)\hiderel{=}C(\omega)^\adjoint$.

% Suppose that $\mathcal{Y}$ is a complex Hilbert space. Since $C(\omega)$ is non-negative, $C(\omega)$ is Hermitian.
