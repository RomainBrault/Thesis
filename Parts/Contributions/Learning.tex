...
Recall that if $B\in\mathcal{L}\left(\mathbb{R}^{u'}, \mathbb{R}^u\right)$ is matrix, the operator $\tildePhi{\omega}(x)$ corresponding to the decomposable kernel is
\begin{dgroup}
\begin{dmath*}
\tildePhi{\omega}(x)y = \frac{1}{\sqrt{D}}\Vect_{j=1}^D\pairing{x, \omega_j}B^T y
\end{dmath*}
\begin{dmath*}
\tildePhi{\omega}(x)^T\theta = \frac{1}{\sqrt{D}} \sum_{j=1}^D \pairing{x, \omega_j}B\theta_j
\end{dmath*}
\end{dgroup*}
